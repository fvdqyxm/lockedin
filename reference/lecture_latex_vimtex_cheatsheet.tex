...
\begin{enumerate}[label=\textcolor{navy}{\arabic*.},leftmargin=1.5em]
  ...
  \item Use \key{;c} for a fresh italic Claim paragraph, \key{;p} for Proof, and \key{;r} for Remark. The \cmd{claim} snippet adds a Claim plus a matched proof in one go.
...
\key{;c} & starts a new paragraph labeled \textit{Claim.}\\
\key{;p} & starts a new paragraph labeled \textit{Proof.}\\
\key{;r} & starts a new paragraph labeled \textit{Remark.}\\
...
\cmd{hw} & \cmd{\textbackslash exercise\{\#.\#\}} header + solution (solutions.tex)\\
\cmd{hwp} & auto-numbered \cmd{Problem N} header + solution (homework files)\\
\cmd{chb} & chapter banner (solutions.tex)\\
\cmd{sol}, \cmd{ans} & solution or answer block\\
\cmd{claim} & claim + proof pair (italic labels, auto \cmd{\textbackslash qed})\\
\cmd{defn} & definition\\
...
\cmd{pset}, \cmd{cond}, \cmd{ev}, \cmd{var}, \cmd{cov} & probability notation\\
\end{tabularx}

\sectiontitle{Good source habits}
\begin{itemize}
  \item Each course folder keeps one growing \cmd{solutions.tex} with a banner per textbook chapter. Jump to the chapter (Toc: \key{Space lt}), type \cmd{hw}, press \key{Tab}, and write.
