...
NOTES_DIR="$REPO_ROOT/lecture_notes"

usage() {
  printf 'Usage: %s COURSE LECTURE_NUMBER\n' "$(basename "$0")" >&2
  printf 'Example: %s Math118 1\n' "$(basename "$0")" >&2
}
...
course_label="$(printf '%s' "$course_input" | sed -E 's/[_-]+/ /g; s/^([[:alpha:]]+)[[:space:]]*([0-9]+)$/\1 \2/')"
course_slug="$(printf '%s' "$course_input" | tr -cd '[:alnum:]')"
lecture_padded="$(printf '%02d' "$((10#$lecture_number))")"
target="$NOTES_DIR/${course_slug}_lecture_${lecture_padded}.tex"
